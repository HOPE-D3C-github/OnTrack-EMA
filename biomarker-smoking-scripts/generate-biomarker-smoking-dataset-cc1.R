# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Summary:  Combine participant-level CC1 biomarker dataframes and apply filtering
#             criteria based on active study-period and de-duplication
#     
# Inputs:   file.path(path_ontrack_ema_staged, "masterlist_updated.RDS")
#           file.path(path_ontrack_ema_staged, "online_puffmarker_episode_raw_data_cc1.RData")
#     
...
load(file.path(path_ontrack_ema_staged, "online_puffmarker_episode_raw_data_cc1.RData"))
...
# -----------------------------------------------------------------------------
crosswalk <- dat_master %>% filter( cc_indicator == 1 ) %>% select(ID_enrolled, participant_id, timezone_local, cc_indicator, first_day_date, last_day_date) %>%
  add_row(dat_master %>% filter( cc_indicator == 1 & !is.na(participant_id_2) ) %>% 
...
save(online_puffmarker_episode_data_cc1, file = file.path(path_ontrack_ema_staged, "online_puffmarker_episode_data_cc1.RData"))
