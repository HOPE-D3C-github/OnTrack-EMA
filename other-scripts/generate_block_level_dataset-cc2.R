...
battery_binned_cc2 <- battery_binned_all %>% filter(cc_indicator == 2)
remove(battery_binned_all)

load(file = file.path(path_ontrack_ema_staged, 'ema_raw_data_cc2.RData'))
phone_status_cc2 <- all_ema_conditions_files_cc2
remove(all_random_ema_response_files_cc2, all_random_ema_status_files_cc2, all_smoking_ema_response_files_cc2, all_smoking_ema_status_files_cc2,
       all_stress_ema_response_files_cc2, all_stress_ema_status_files_cc2, dat_file_counts_cc2, all_ema_conditions_files_cc2)
...
block_level_ema_cc2 <- block_level_ema_cc2 %>% relocate(status, begin_hrts_local, end_hrts_local, block_start_hrts_local, block_end_hrts_local, day_start_hrts_local, day_end_hrts_local, .after = block)
...
if(F){block_level_ema_cc2 %>% count(no_day_start_on_study_day, status)}
...
#
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
if(F){block_level_ema_cc2 %>% mutate(on_withdrew_date = study_date == withdrew_date) %>% View}
if(F){block_level_ema_cc2 %>% mutate(on_withdrew_date = study_date == withdrew_date, .after = study_date) %>% 
    relocate(withdrew_date, .after = study_date) %>% filter(on_withdrew_date) %>% count(status)}

block_level_ema_cc2 <- block_level_ema_cc2 %>% mutate(
...
block_level_ema_cc2 %>% count(after_pt_withdrew, status)
...
#   * Battery Status with a value of NA represents a day without a day start button press if the participant has other battery data, otherwise its a participant without battery data
...
battery_binned_cc2 <- battery_binned_cc2 %>% 
  select(ID_enrolled, battery_hrts_local_start, battery_hrts_local_end, battery_status)


ema_dat_delivered <- block_level_ema_cc2 %>% filter(status != 'UNDELIVERED') %>% mutate(ema_delivered = TRUE)
ema_dat_undelivered <- block_level_ema_cc2 %>% filter(status == 'UNDELIVERED') %>% mutate(ema_delivered = FALSE)

test_1 <- test_that(desc = 'Test that the split grabbed all the rows from block_level_ema_cc2', {
  expect_equal(object = nrow(ema_dat_delivered) + nrow(ema_dat_undelivered),
               expected = nrow(block_level_ema_cc2))
})
...
ema_dat_undelivered <- left_join( x = ema_dat_undelivered, y = battery_binned_cc2, by)
remove(by)
...
if(F){ema_dat_undelivered %>% count(battery_status, no_day_start_on_study_day)}
...
if(F){ema_dat_undelivered %>% count(undelivered_rsn) %>% mutate(percent = n/sum(n)*100)} # >85% undelivered EMAs are reconciled from the above
...
crosswalk_id_enrolled <- dat_master %>% filter(cc_indicator == 2) %>% select(ID_enrolled, participant_id, timezone_local) %>% 
  add_row(
    dat_master %>% filter(cc_indicator == 2 & !is.na(participant_id_2)) %>% select(ID_enrolled, participant_id = participant_id_2, timezone_local)
  )

phone_status_cc2 <- phone_status_cc2 %>%     # Known issue that 'ses_119' was the participant ID given to participants. Will likely need to remove data since can't determine which pt each data point came from
  left_join( y = crosswalk_id_enrolled,
             by = 'participant_id' )

phone_status_cc2 <- phone_status_cc2 %>% 
...
phone_status_cc2 <- phone_status_cc2 %>% 
  mutate(hrts_UTC = as_datetime(mCerebrum_timestamp/1000), .after = mCerebrum_timestamp) %>% 
  mutate(hrts_local = map2(hrts_UTC, timezone_local,
                           ~with_tz( .x, .y) %>% force_tz( ., "UTC") ) %>% unlist %>% as_datetime(.), .after = hrts_UTC )

phone_status_cc2 <- phone_status_cc2 %>% 
  mutate(ema_type = str_split(conditional_type, pattern = 'EMA-', simplify = TRUE)[,2], .after = conditional_type) %>%  # Create variable from string id which indicates the EMA type
  select(-conditional_type)                                                                               # remove the id variable after creating ema_type from it


phone_status_cc2 <- phone_status_cc2 %>% 
  filter(ema_type == 'RANDOM') %>%                              # Remove rows not pertinent to Random EMAs
  select(-block_number, -mCerebrum_offset) %>%                  # Remove study block variable, we calculate one more robustly from dat_master
  rename(unixtime = mCerebrum_timestamp)
  
phone_status_cc2 <- phone_status_cc2 %>% rename_with(.fn = ~paste0('phone_log_', .x), .cols = 2:ncol(.))
...
undelivered_not_reconciled <- right_join(x = phone_status_cc2,
...
if(F){undelivered_not_reconciled %>% count(phone_log_is_driving)} # can use regex to grab true, false, 0 value from is_driving variable
if(F){undelivered_not_reconciled %>% count(phone_log_is_privacy_on)} # can use regex to grab true, false, 0 value from is_privacy_on variable
if(F){undelivered_not_reconciled %>% count(phone_log_GET_PHONE_BATTERY)} # can use regex to grab battery value and run logic against greater than 10


undelivered_not_reconciled <- undelivered_not_reconciled %>% 
  mutate(driving_cleaned = (str_split_fixed(phone_log_is_driving, pattern = ' \\[', n = 2)[,1]) == 'true', 
         privacy_on_cleaned = (str_split_fixed(phone_log_is_privacy_on, pattern = ' \\[', n = 2)[,1]) == '1',
         low_battery_cleaned = as.numeric(str_split_fixed(phone_log_GET_PHONE_BATTERY, pattern = ' \\[', n = 2)[,1]) <= 10,
         .after = block) 
...
undelivered_not_reconciled <- undelivered_not_reconciled %>% 
  ...
  mutate(
    n_insuf_battery = sum(low_battery_cleaned, na.rm = TRUE),
    n_driving = sum(driving_cleaned, na.rm=TRUE),
    n_privacy_mode = sum(privacy_on_cleaned, na.rm = TRUE),
    .after = block
    ) %>% 
  filter(row_number() == 1) %>% 
  ungroup()
...
colnames_of_interest = c('n_insuf_battery', 'n_driving', 'n_privacy_mode')
...
# End of Step 4
remove(phone_status_cc1, crosswalk_id_enrolled)
...
undelivered_rsn_block_level_ema_cc2 <- ema_dat_delivered %>% add_row(undelivered_reconciled %>% select(colnames(ema_dat_delivered)))

if(F){undelivered_rsn_block_level_ema_cc2 %>% count(status, ema_delivered)}


if(all(test_1, test_2, test_3, test_4)){
  print("Passed all tests. Saving data as block_level_ema_dataset_cc2.RDS")
  saveRDS(undelivered_rsn_block_level_ema_cc2,
          file = file.path(path_ontrack_ema_staged, "block_level_ema_dataset_cc2.RDS"))
} else {
