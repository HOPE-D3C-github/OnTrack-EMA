...
data_all <- data_all %>% 
  mutate(enrolled = case_when(
    ...
    T~F)) %>% 
  mutate(cc_indicator = case_when(
    enrolled & is.na(ID_enrolled) ~ "1",
    T ~ cc_indicator
  )) %>% 
  mutate(ID_enrolled = case_when(  # Add in an ID_enrolled for "enrolled" participants who were not assigned a Grafana ID
    enrolled & is.na(ID_enrolled) ~ ID_RSR,
    T ~ ID_enrolled
  ), 
  participant_id = case_when(
    enrolled & is.na(participant_id) ~ ID_RSR,
    T ~ participant_id))
...
write_csv(crswlk_export, file = file.path(path_ontrack_visit_outputs, "utah_crosswalk.csv"))
...
write_csv(data_all, file = file.path(path_ontrack_visit_outputs, "utah_data_all.csv"))
...
write_csv(chk, file =  file.path(path_ontrack_visit_outputs, "utah_data_checklist_clean.csv"))
...
write_csv(quest, file = file.path(path_ontrack_visit_outputs, "utah_questionnaire.csv"))
...
write_csv(scr, file = file.path(path_ontrack_visit_outputs, "utah_screener.csv"))
...
write_csv(event_wide, file = file.path(path_ontrack_visit_outputs, "utah_event_clean_wide.rds"))
...
write_csv(visit_comments, file = file.path(path_ontrack_visit_outputs, "utah_visit_comments.rds"))
